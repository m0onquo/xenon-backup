...
--// V3 UI Library
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/m0onquo/xenon-backup/refs/heads/main/UI_LIB.lua"))()
...
function Xenon.Utils:ReadData()
    ...
    Data = Data or loadstring(game:HttpGet("https://raw.githubusercontent.com/m0onquo/xenon-backup/refs/heads/main/DATA.lua"))()
    return {
        Data = Data;
        Success = Success;
        Error = Error;
...
function Xenon.Utils:FindCaseInsensitive(TableName, SearchValue)
    local tbl = self.Tables[TableName]
    if not tbl or not tbl.Value then return nil end
    local searchLower = string.lower(SearchValue)
    for i, v in pairs(tbl.Value) do
        if type(v) == "string" and string.lower(v) == searchLower then
            return i, v
        end
    end
    return nil
...
function Xenon.Utils:AddESPForExistingItems()
        ...
        if ItemsFolder then
            for _, Model in pairs(ItemsFolder:GetChildren()) do
                if Model:IsA("Model") and Model.PrimaryPart then
                    ...
                    if Prompt and Prompt.MaxActivationDistance == 8 then
                        local IdentifiedItem = Prompt.ObjectText
                        
                        -- Create ESP for existing item
                        local ESPPart = Instance.new("Part", workspace)
                        ESPPart.Name = IdentifiedItem .. "_Existing"
                        ...
                        task.spawn(function()
                            while task.wait(0.1) do
                                if Model.Parent == workspace.Item_Spawns.Items and Model.PrimaryPart then
                                    if table.find(self:GetTable("Items"), IdentifiedItem) and self:GetState("Item ESP") == true then
                                        Billboard.Enabled = true
                                        ESPLabel.Text = IdentifiedItem .. " (" .. self:GetDistance(Model.PrimaryPart) .. "m)"
                                    else
                                        Billboard.Enabled = false
                                    end
                                else
...
function Xenon.Utils:Stats()
    ...
    if self:GetState("Rib Farm") == true or self:GetState("Rib Shiny Farm") == true then
        -- В игре теперь только один уровень Worthiness
    end
...
function Xenon.Utils:FarmItems(List)
    -- Собираем из очереди (новые предметы)
    ...
    -- Сканируем все существующие предметы (старые предметы)
    local ItemSpawns = workspace:FindFirstChild("Item_Spawns")
    if ItemSpawns then
        local ItemsFolder = ItemSpawns:FindFirstChild("Items")
        if ItemsFolder then
...
function Xenon.Utils:PressKey(KeyCode)
    local Character = self:GetCharacter()
    if Character and Character:FindFirstChild("RemoteEvent") then
        Character:FindFirstChild("RemoteEvent"):FireServer("InputBegan", {
            ["Input"] = KeyCode
        })
    end
end

function Xenon.Utils:PressY()
    self:PressKey(Enum.KeyCode.Y)
end

function Xenon.Utils:PressZ()
    self:PressKey(Enum.KeyCode.Z)
end

function Xenon.Utils:ToggleGodMode(State)
    self:SetState("God Mode", State)

    if self:GetState("God Mode") == true then
        pcall(function()
            SpeedToggle:UpdateAsset(true)
            AutoSprintToggle:UpdateAsset(true)
        end)

        Library:Notification("Godmode", "Godmode has been enabled", 5)

        self:AddTask("RemoveHealthText", workspace.IgnoreInstances.ChildAdded:Connect(function(child)
            task.wait()
            if child.Name == "Part" then
                pcall(function()
                    child.HealthText:Destroy()
                end)
            end
        end))

        self:AddTask("Godmode", self:GetPlayer().Character.Humanoid.HealthChanged:Connect(function(val)
            self:GetCharacter().RemoteEvent:FireServer("Poison", {Duration = math.huge, TotalDamage = math.huge})
        end))

        self:AddTask("SpoofHealth", self:GetService("RunService").Heartbeat:Connect(function()
            if self:GetPlayer().PlayerGui:FindFirstChild("HUD") then
                self:GetPlayer().PlayerGui.HUD.Main.Indicators.Bars.Health.Size = UDim2.new(0.638530254, 0, 0.475938797, 0)
            end
        end))
    else
        pcall(function()
        ...
        self:DisconnectTask("Godmode")
        self:DisconnectTask("SpoofHealth")
        task.wait(0.2)
        self:DisconnectTask("RemoveHealthText")
        self:GetHumanoid().Health = 0
    end
end

function Xenon.Utils:SetAutoArcade(State)
    getgenv().AutoArcade = (State == true)
    if State then
        local last = 0
        self:AddTask("AutoArcade", self:GetService("RunService").Heartbeat:Connect(function()
            if getgenv().AutoArcade ~= true then
                return
            end

            local player = self:GetPlayer()
            local character = self:GetCharacter()
            if not player or not character or not character:FindFirstChild("RemoteEvent") then
                return
            end
            ...
            local RollingGUI = gui:FindFirstChild("RollingItem")
            if RollingGUI then
                RollingGUI.Enabled = false
                return
            end
        ...
        self:DisconnectTask("AutoArcade")
    end
end

function Xenon.Utils:YAbility(TargetModel)
    ...
    local fc = Character:FindFirstChild("FocusCam")
    if fc then
        pcall(function()
            fc:Destroy()
        end)
...
function Xenon.Utils:FindPlayerByPartialName(Query)
    local q = string.lower(tostring(Query or ""):gsub("^%s+", ""):gsub("%s+$", ""))
    if q == "" then
        return nil
    end

    for _, plr in pairs(self:GetService("Players"):GetPlayers()) do
...
function Xenon.Utils:GetClosestPlayer()
    local closest = nil
    ...
    for _, plr in pairs(self:GetService("Players"):GetPlayers()) do
        if plr ~= self:GetPlayer() and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
...
function Xenon.Utils:AttemptKidnap(TargetModel)
    if not TargetModel then return end
    ...
    if Character:FindFirstChild("SummonedStand") and Character:FindFirstChild("SummonedStand").Value == false then
        self:EquipStand()
    end

    local OldCFrame = HRP.CFrame

    if TargetModel:FindFirstChild("InCocoJumbo") then
        return
    end
    ...
    if Character:FindFirstChild("RemoteEvent") then
        Character:FindFirstChild("RemoteEvent"):FireServer("InputBegan", { ["Input"] = Enum.KeyCode.E, ["HoldW"] = true })
    end

    local TeleportLoop
    TeleportLoop = self:GetService("RunService").PreRender:Connect(function()
        if not TargetModel.Parent then return end
        TargetHRP = TargetModel:FindFirstChild("HumanoidRootPart")
        if not TargetHRP then return end

        HRP.Velocity = Vector3.new()

        local head = TargetModel:FindFirstChild("Head")
        local lookTarget = head and head.Position or (TargetHRP.Position + TargetHRP.CFrame.LookVector * 5)
        local frontPos = TargetHRP.Position + TargetHRP.CFrame.LookVector * 1
        HRP.CFrame = CFrame.lookAt(frontPos, lookTarget)
    end)

    TargetModel:WaitForChild("InCocoJumbo", 2)
...
function Xenon.Utils:LetOutPlayer(TargetModel, TargetCFrame)
    if not TargetModel then return end
    local Character = self:GetCharacter()
    if not Character then return end
    local HRP = self:GetHRP()
    ...
    TeleportLoop = self:GetService("RunService").PreRender:Connect(function()
        ...
        if Character:FindFirstChild("RemoteEvent") then
            Character:FindFirstChild("RemoteEvent"):FireServer("InputBegan", { ["Input"] = Enum.KeyCode.Z, ["HoldW"] = true })
        end
    end)
...
function Xenon.Utils:KidnapPlayerByQuery(Query)
    local targetPlr = self:FindPlayerByPartialName(Query)
    if not targetPlr or not targetPlr.Character then
        return false
    end

    self:AttemptKidnap(targetPlr.Character)
    self:LetOutPlayer(targetPlr.Character, CFrame.new(0, -500, 0))
    return true
end
        ...
        while (Enemy and EnemyHealth and EnemyHealth.Value > 0 and EnemyHRP and EnemyHumanoid) do
            ...
            if Character and Character:FindFirstChildWhichIsA("Humanoid") and Character:FindFirstChildWhichIsA("Humanoid").Health > 0 then
                    ...
                    if self:CheckStand() ~= "None" then
                        ...
                        local npcFarmYOffset = -35
                        if self:GetState("NPC Farm") == true and self:GetString("Target NPC") == "The Idol" then
                            npcFarmYOffset = 35
                        end
                        Character.PrimaryPart.CFrame = Character.StandMorph.PrimaryPart.CFrame + Character.StandMorph.PrimaryPart.CFrame.LookVector * math.random(-3,-2) + Vector3.new(0, npcFarmYOffset, 0) 
                    else
                    ...
                    task.spawn(function()
                            self:UseMove("m1")
                            -- Auto Skills
                            for _, key in pairs(self:GetTable("AutoSkills")) do
                                local keyCode = Enum.KeyCode[key]
                                if keyCode then
                                    self:UseMove(keyCode)
                                end
                            end
                        end)
...
local WSJP_Connection = nil
local function WSJP_Stop()
    if WSJP_Connection then
        WSJP_Connection:Disconnect()
        WSJP_Connection = nil
    end
end

local function WSJP_Start()
    if WSJP_Connection then return end
    WSJP_Connection = Util:GetService("RunService").Heartbeat:Connect(function()
        local Humanoid = Util:GetHumanoid()
        if not Humanoid then return end

        if Util:GetState("Jump") == true then
            Humanoid.JumpPower = Util:GetInt("Jump")
        end
    end)
end

local function WSJP_Refresh()
    if Util:GetState("Jump") == true then
        WSJP_Start()
    else
        WSJP_Stop()
...
local function Speed_Stop()
    ...
    pcall(function()
        ...
        if HRP then
            local Existing = HRP:FindFirstChild("Xenon_SpeedBV")
            if Existing then
                Existing:Destroy()
            end
        end
...
local function Speed_Start()
    ...
    Speed_Connection = Util:GetService("RunService").Heartbeat:Connect(function()
        ...
        local Humanoid = Util:GetHumanoid()
        local HRP = Util:GetHRP()
        if not Humanoid or not HRP or Humanoid.Health <= 1 then
            Speed_Stop()
            return
        end

        if not SpeedBV or SpeedBV.Parent ~= HRP then
            SpeedBV = HRP:FindFirstChild("Xenon_SpeedBV")
            if not SpeedBV then
                SpeedBV = Instance.new("BodyVelocity")
                SpeedBV.Name = "Xenon_SpeedBV"
                SpeedBV.Parent = HRP
            end
        end

        SpeedBV.MaxForce = Vector3.new(math.huge, 0, math.huge)
        SpeedBV.Velocity = Humanoid.MoveDirection * Util:GetInt("Speed")

        for _, v in pairs(HRP:GetChildren()) do
            if v:IsA("BodyVelocity") and v ~= SpeedBV then
                SpeedBV.Velocity = v.Velocity
                break
            end
        end
    end)
...
if not Util:IsSBR() and game.PlaceId ~= 5290908008 then
    ...
    -- Добавить ESP для существующих предметов
    Util:AddESPForExistingItems()
...
Util:AddValues{
    ...
    ["Table"] = {
        ...
        AllItems = {"Christmas Present", "Mysterious Arrow", "Pure Rokakaka", "Rokakaka", "Diamond", "Lucky Arrow", "Lucky Stone Mask", "Dio's Diary", "Steel Ball", "Rib Cage of The Saint's Corpse", "Stone Mask", "Gold Coin", "Quinton's Glove", "Ancient Scroll", "Caesar's Headband", "Zeppeli's Hat", "Clackers"};
        ...
        AllShinys = {
            ...
            "BIBLICALY ACCURATE GE",
            "Bites The Ice",
            "BLADE OF EXILE",
            "Blade Of The Exile",
            "Bloodthirster",
            "Bloodthirster Anubis",
            "Booette",
            "Booette STW",
            "Buggy D. Clown",
            "Candy Cane World",
            ...
            "Festive Free",
            "FESTIVE PLATINUM",
            "Festive Platinum SPTW",
            "FESTIVE SPTW",
            "Festive Star Platinum",
            "FESTIVE THE WORLD",
            "Festive TWOH",
            "Forged in Hell",
            "Frost HG",
            ...
            "Grim Reaper",
            "Grim Reaper KCR",
            "Haunted Jack-O-Platinum",
            "Haunted Jack-O-Platinum: The World",
            "Headhunter",
            "Heaven Act 4",
            "Heaven Spirit",
            "HEAVEN'S ACT 4",
            ...
            "Hidden Leaf Hero",
            "Hidden Leaf Hero LOVE TRAIN",
            "Hierophant Frost",
            "Holiday Tangle",
            "Holly's Sickness",
            "HORSEMAN OF HEAVEN",
            "Horsemen Of Heaven",
            "Ice Cold Chili Pepper AU",
            ...
            "Mintsnake",
            "MIRAGE OF PHANTOMS",
            "Mirage Of Phantoms TWOH",
            "Misaka",
            "Misaka Mikoto",
            "Molted Chariot",
            "Moon of Christmas Present",
            "MR JOESTAR",
            "Mr Juke's Angel",
            "Mr Jukes Angel",
            "Mr Pekin",
            ...
            "Pumpkin Patch",
            "Pumpkin Patch SF",
            "PUMPKIN PATCH STONE FREE",
            "Pyro",
            "Pyro MR",
            "Queen Crimson",
            "Retro Scary Monsters",
            ...
            "Shanks SCR",
            "Sheared Snow Golem",
            "Sheared Snow Golem MR",
            "SMTWAU",
            "Snake of Christmas Past",
            "Snow Golem",
            "Snow Golem MR",
            "SOCP MIH",
            "Soft and Starry",
...
Credits_Developers:Label("Fork By: Danita11311")
...
Player_Combat:TextBox("Kidnap Target", function(txt)
    Util:SetString("Kidnap Target", txt)
    Util:SaveConfig();
end, "Enter part of a player's name")

Player_Combat:Button("Mr Pres Kidnap (Target)", function()
    local q = Util:GetString("Kidnap Target")
    local ok = Util:KidnapPlayerByQuery(q)
    if not ok then
        Library:Notification("Kidnap", "Target not found / no character", 3)
    end
end, "Kidnap player by partial name")

Player_Combat:Button("Mr Pres Kidnap (Closest)", function()
    local plr = Util:GetClosestPlayer()
    if plr and plr.Character then
        Util:AttemptKidnap(plr.Character)
        Util:LetOutPlayer(plr.Character, CFrame.new(0, -500, 0))
    else
        Library:Notification("Kidnap", "No closest player", 3)
    end
end, "Kidnap the closest player")

Player_Combat:Button("Beach Boy Fling (Target)", function()
    local q = Util:GetString("Kidnap Target")
    local plr = Util:FindPlayerByPartialName(q)
    if plr and plr.Character then
        Util:YAbility(plr.Character)
    else
        Library:Notification("Y Ability", "Target not found / no character", 3)
    end
end, "Uses Y ability on partial nickname target")

Player_Combat:Button("Beach Boy Fling (Closest)", function()
    local plr = Util:GetClosestPlayer()
    if plr and plr.Character then
        Util:YAbility(plr.Character)
    else
        Library:Notification("Y Ability", "No closest player", 3)
    end
end, "Uses Y ability on closest player")

Player_Combat:Toggle("Auto Arcade + God Mode", false, function(State)
    Util:SetAutoArcade(State)
    Util:ToggleGodMode(State)
end)
...
Autofarms_ItemFarm:Toggle("Item Farm", Util:GetState("Item Farming"), function(State)
    ...
    if State == true then
        Util:Teleport(CFrame.new(1170, 209, -28))
...
Autofarms_StandFarm:Toggle("Rib Farm", Util:GetState("Rib Farming"), function(State)
    ...
    if Util:GetState("Rib Farm") == true then
        Util:GetHRP().CFrame = CFrame.new(1170, 209, -28)
...
for i,v in pairs(Util:GetTable("AllShinys")) do
    Util:InsertTable("ShinyToggles", Autofarms_Shinys:Toggle(v, (Util:FindCaseInsensitive("Shinys", v) ~= nil and true or false) , function(State)
        if State == true then
            if not Util:FindCaseInsensitive("Shinys", v) then
                Util:InsertTable("Shinys", v)
            end
        else
            local idx = Util:FindCaseInsensitive("Shinys", v)
            if idx then
                Util:RemoveTable("Shinys", Util:GetTable("Shinys")[idx])
            end
        end
        Util:SaveConfig();
...
--// Auto Skills
local AutoSkillsLabel = Autofarms_NpcFarm:Label("Skills: None")
local AddSkillToggleRef = nil

AddSkillToggleRef = Autofarms_NpcFarm:Toggle("Add Skill", false, function(State)
    if State then
        local UIS = game:GetService("UserInputService")
        local connection
        connection = UIS.InputBegan:Connect(function(input, gameProcessed)
            ...
            if keyName and keyName ~= "Unknown" then
                if not table.find(Util:GetTable("AutoSkills"), keyName) then
                    Util:InsertTable("AutoSkills", keyName)
                    local skills = Util:GetTable("AutoSkills")
                    AutoSkillsLabel:UpdateAsset("Skills: " .. table.concat(skills, ", "))
                end
                connection:Disconnect()
                if AddSkillToggleRef then
                    AddSkillToggleRef:UpdateAsset(false)
                end
            end
        end)
    end
end, "Toggle on, press a key to add it (auto-disables after)")

Autofarms_NpcFarm:Toggle("Clear Skills", false, function(State)
    if State then
        local t = Util:GetTable("AutoSkills")
        for i = #t, 1, -1 do
            t[i] = nil
        end
        AutoSkillsLabel:UpdateAsset("Skills: None")
    end
end, "Clear all auto skills")
...
Misc_Arcade:Toggle("Auto Arcade + God Mode", false, function(State)
    Util:SetAutoArcade(State)
    Util:ToggleGodMode(State)
end)
